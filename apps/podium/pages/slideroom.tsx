...
const Slideroom: FC<slideroomProps> = ({ }) => {
  const { ranked, unranked, onHold, projects, spotlight } = useContext(ProjectContext);
  ...
  useEffect(() => {
    setIsLoading(true)
    setSpotlightProject(allProjects.find(p => p.projectId === router.query.spotlightId));
    setIsLoading(false)
  }, [])
      ...
      <DndContext
            ...
            if(active.data.current?.type === 'Spotlight') {
              switch(over.data.current?.type) {
                case 'Ranked':
                  ...
                  const rankedIndex = rankedProjects.findIndex(({ projectId }) => projectId === active.id);
        ...
        onDragCancel={() => {
          ...
          <div className={styles.spotlightContainer}>
            {spotlightProject ? <SlideroomSpotlight project={spotlightProject} /> : "You're all caught up!"}
          </div>
          <div className={styles.slideroomProjectBar}>
            <div>
              <p>On Hold will be added later (yes this will put multiple copies of the same project in onhold as of now)</p>
            ...
            <div>
              <SortableContext items={allProjectIds}>
                ...
                {rankedProjects.map((project) => (
                  spotlightProject && spotlightProject.projectId === project.projectId ? (
                    <></>
                  ) : (
                    <SlideroomRankProject
                      key={project.projectId}
                      project={project}
                      ranking={rankedProjects.findIndex(p => p.projectId === project.projectId) + 1}
                      type={'Ranked'}
                    />
                  )
                ))}
