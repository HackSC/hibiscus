...
def get_pinned_events(user_id: str):
    res = supabase.table("pinned_events").select("events(name, description, start, end)").eq("user_id", user_id).execute();
    return res.data
...
def remove_pin(user_id: str, event_id: int):
    res = supabase.table("pinned_events").delete().eq("user_id", user_id).eq("event_id", event_id).execute()
    return res.data
